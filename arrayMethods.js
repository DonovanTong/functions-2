...
const evenNumbers = mixedNumbers.filter((item) => {
  return item % 2 === 0
})
...
// CODE HERE
const totalPopulation  = populations.reduce((a, c) => {
  return a + c
})
...
const mappedArr = orders.map((orders) => {
...
const bobsPurchases = purchases.reduce((a, c) => {
  if (c.owner === 'Bob') {
    return a+c.price
  } else {
    return a
